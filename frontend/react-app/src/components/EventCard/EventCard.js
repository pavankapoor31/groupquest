...
const EventCard = ({ event }) => {
  ...
  profileId = JSON.parse(profileId);
    const [isGoingClicked, setGoingClicked] = useState(event.members && event.members.includes(profileId));
  ...
  const [memberCount, setMemberCount] = useState(event.members?.length);
  ...
  const handleGoingClick = () => {
    ...
    if (!isGoingClicked) {
      ...
      if(event.members)
       payload = [...event.members,profileId]
      // http://localhost:3001/api/Events/update?where={"id":${JSON.stringify(id)}}
      axios.post(` http://localhost:3001/api/events/update?where={"id":"${event.id}"}`, { "members": payload })
        .then(() => {
    ...
    <Card sx={{ maxWidth: 350, margin: '16px', boxShadow: '0 4px 8px rgba(0, 0, 0, 0.1)' }}>
      ...
      <CardContent>
        ...
        <div className='pull-right'>
          <ShareIcon className='mr-3' role="button" onClick={handleShareClick}/>
